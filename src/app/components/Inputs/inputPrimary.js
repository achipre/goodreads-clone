...
export const InputPrimary = ({ type, placeholder, className, icon, label }) => {
  ...
  return (
    <div className="relative">
      ...
      {icon}
      {/* Error campo vacio */}
      {(label === 'fullname' && errors?.fullname?.type === 'required') && <span className={`${poppisBold.className} text-xs absolute block text-millbrook-500 animate-fade-right`}>* Field required</span>}
      ...
      {/* Error otros */}
      {(label === 'fullname' && errors?.fullname?.type === 'minLength') && <span className={`${poppisBold.className} text-xs  absolute block text-millbrook-500 animate-fade-right `}>* Enter minimum 5 characters</span>}
